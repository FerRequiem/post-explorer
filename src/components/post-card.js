...
class PostCard extends LitElement {
    ...
    .chevron.open { transform: rotate(180deg); color: var(--accent2); }
  ...
  `;
    ...
    render() {
        ...
        const excerpt = this.post.body.length > 100
      ...
      <article class="card ${this.expanded ? 'expanded' : ''}">
        ...
        >
          <div>
            <div class="post-id">#${this.post.id} · user ${this.post.userId}</div>
        ...
        ${this.expanded ? html`
          <div class="card-body">
            ...
            ${this.error ? html`
              <div class="inner-error">⚠ ${this.error}</div>
            ` : ''}
