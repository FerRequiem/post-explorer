  ...
  `;
    ...
    _renderPagination() {
      ...
      <div class="pagination" role="navigation" aria-label="Paginación">
        <button
          ...
          @click=${() => { this.page--; window.scrollTo(0,0); }}
        ...
        ${pages.map(p => html`
          <button
            ...
            @click=${() => { this.page = p; window.scrollTo(0,0); }}
    ...
    render() {
      ...
      <div class="app">
        ...
        ${this.showStats ? html`
          <stats-panel .posts=${this.posts}></stats-panel>
        ` : ''}
        ...
        <div class="toolbar">
          ...
          <span class="count">
            <strong>${this.filtered.length}</strong> / ${this.posts.length} posts
          </span>
        </div>
        ...
        ${this.loading ? html`
          <div class="skeletons">
            ${Array(PAGE_SIZE).fill(0).map(() => html`<div class="skeleton"></div>`)}
          </div>
        ` : ''}

        <!-- Sin resultados -->
        ${!this.loading && this.filtered.length === 0 ? html`
          <div class="empty">No se encontraron posts para "<strong>${this.query}</strong>"</div>
        ` : ''}

        <!-- Grid de posts -->
        ${!this.loading ? html`
          <div class="posts-grid">
            ${this._paginated.map(post => html`
